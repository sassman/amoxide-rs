_am() {
    ...
    case "${cmd}" in
        am)
            opts="-h -V --help --version add remove ls la status profile var init setup use tui export import share trust untrust sync __update-check help"
        ...
        am__subcmd__export)
            ...
            ;;
        am__subcmd__help)
            opts="add remove ls la status profile var init setup use tui export import share trust untrust sync __update-check help"
            if [[ ${cur} == -* || ${COMP_CWORD} -eq 2 ]] ; then
                COMPREPLY=( $(compgen -W "${opts}" -- "${cur}") )
                return 0
            fi
