    ...
    <div class="gallery">
        ...
        <div class="tiles">
            <template v-for="profile in filtered" :key="profile.slug">
                <div :class="['tile', { 'tile-selected': viewSlug === profile.slug }]">
                    <button
                        class="tile-expand-btn"
                        @click.stop="openDetail(profile.slug)"
                        title="Expand"
                    >
                        ↗
                    </button>
                    ...
                    <details
                        class="tile-toml"
                        @click.stop
                        @toggle="onTileToggle(profile.slug, $event)"
                    >
                        <summary>
                            View aliases ({{ profile.profiles.length }}
                            {{ profile.profiles.length === 1 ? "profile" : "profiles" }},
                            {{ profile.aliasCount }}
                            {{ profile.aliasCount === 1 ? "alias" : "aliases" }})
                        </summary>
                        <div class="toml-block">
                            <pre
                                v-if="tileTomlCache[profile.slug]"
                            ><code>{{ tileTomlCache[profile.slug] }}</code></pre>
                    ...
                    <div v-if="expandedImport === profile.slug" class="import-detail" @click.stop>
                        ...
                        />
                        <p class="import-hint">
                            <a :href="originUrl(profile)" target="_blank"
        ...
        <div v-if="filtered.length === 0" class="empty">No profiles found.</div>
            ...
            <div v-if="viewProfile" class="modal-overlay" @click.self="closeDetail()">
                <div class="modal-content" ref="detailRef">
                    ...
                    <div class="detail-byline">
                        by
                        <a :href="`https://github.com/${viewProfile.author}`" target="_blank">{{
                            viewProfile.author
                        }}</a>
                        <template v-if="viewProfile.profiles.length">
                            · provides
                            <template v-for="(p, i) in viewProfile.profiles" :key="p"
                                ><template v-if="i > 0">, </template><code>{{ p }}</code></template
                            ></template
                        >
                        · {{ viewProfile.aliasCount }}
                        {{ viewProfile.aliasCount === 1 ? "alias" : "aliases" }} ·
                    ...
                    <div class="detail-scroll">
                        ...
                        <div v-if="viewDetail" class="detail-section">
                            <h3>
                                Aliases ({{ viewProfile.profiles.length }}
                                {{ viewProfile.profiles.length === 1 ? "profile" : "profiles" }},
                                {{ viewProfile.aliasCount }}
                                {{ viewProfile.aliasCount === 1 ? "alias" : "aliases" }})
                            </h3>
                    ...
                    <div class="detail-footer">
                        ...
                        <div v-if="modalExpandedImport === viewProfile.slug" class="import-detail">
                            ...
                            />
                            <p class="import-hint">
                                <a :href="originUrl(viewProfile)" target="_blank"
                                    >See the original source</a
                                >
                                before running this command.
                            </p>
